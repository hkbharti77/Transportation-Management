...
export default function SignInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loginData = await userService.login(formData.username, formData.password);
        ...
        setTimeout(() => {
          switch (currentUser.role.toLowerCase()) {
            ...
            case 'public_service_manager':
              ...
              break;
            case 'dispatcher':
              window.location.href = '/dispatcher-home';
              break;
            case 'staff':
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(data.message || "OTP sent successfully to your email. Redirecting to password reset page...");
      
      // Redirect to the dedicated forgot password page with the email pre-filled
      setTimeout(() => {
        window.location.href = `/forgot-password?email=${encodeURIComponent(resetEmail)}`;
      }, 2000);
    } catch (err: unknown) {
      ...
      <div className="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
        <div>
          ...
          <div>
            ...
            {showForgotPassword ? renderForgotPasswordForm() : renderSignInForm()}

            <div className="mt-5">
              <p className="text-sm font-normal text-center text-gray-700 dark:text-gray-400 sm:text-start">
                {showForgotPassword ? "Remember your password?" : "Don't have an account?"} {""}
                {showForgotPassword ? (
                  <button
                    onClick={() => setShowForgotPassword(false)}
