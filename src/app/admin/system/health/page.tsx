      ...
      <ComponentCard title="Overall System Status">
        <div className="p-6 text-center">
          <div className="text-6xl mb-4">
            {healthData.length > 0 && healthData.every((service: any) => service.status === 'healthy') ? '✅' : '⚠️'}
          </div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
            {healthData.length > 0 && healthData.every((service: any) => service.status === 'healthy') ? 'All Systems Operational' : 'System Issues Detected'}
          </h2>
          <p className="text-gray-600 dark:text-gray-400">
            {healthData.length > 0 ? `${healthData.length} services monitored` : 'No services currently monitored'}
          </p>
        </div>
      </ComponentCard>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="text-center p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
              ...
              <p className="text-3xl font-bold text-green-600 dark:text-green-400">
                {healthData.filter((service: any) => service.status === 'healthy').length}
              </p>
            ...
            <div className="text-center p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">
                {healthData.length > 0 
                  ? Math.round(healthData.reduce((sum: number, service: any) => sum + service.response_time, 0) / healthData.length) 
                  : 0}ms
              </p>
