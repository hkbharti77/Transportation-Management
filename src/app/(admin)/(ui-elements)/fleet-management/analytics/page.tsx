...
interface FleetAnalyticsData {
  ...
  idle_trucks: number;
  utilization_by_truck?: TruckUtilization[];
}

export default function FleetAnalyticsPage() {
  const { user, isAuthenticated } = useAuth();
  const [analyticsData, setAnalyticsData] = useState<FleetAnalyticsData | null>(null);
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/v1/admin/analytics/truck-utilization', {
        method: 'GET',
        headers: {
          ...
          <div className="p-6">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {analyticsData.utilization_by_truck.map((truck: TruckUtilization, index: number) => (
