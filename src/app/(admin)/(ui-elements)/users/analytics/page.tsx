...
// Define interfaces for the user analytics data
...
export default function UserAnalyticsPage() {
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/v1/admin/users/analytics', {
        method: 'GET',
        headers: {
    ...
    <div className="space-y-6">
        ...
        <ComponentCard title="User Growth Trend">
          <div className="p-4 text-center">
            ...
            <p className="text-2xl font-bold text-teal-600 dark:text-teal-400">
              {analyticsData.user_growth.filter((item: UserGrowthItem) => item.count > 0).length} days
            </p>
      ...
      {analyticsData.user_growth && analyticsData.user_growth.length > 0 && (
        <ComponentCard title="User Growth (Last 7 Days)">
          <div className="p-6">
            <div className="space-y-4">
              {analyticsData.user_growth.map((growth: UserGrowthItem, index: number) => {
                return (
                    ...
                    <div className="flex-1 mx-4">
                      ...
                      <div className="bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                        <div 
                          ...
                          style={{ width: `${(growth.count / Math.max(...analyticsData.user_growth.map((g: UserGrowthItem) => g.count))) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
