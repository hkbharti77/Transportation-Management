...
export default function RevenueAnalyticsPage() {
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/api/v1/admin/analytics/revenue', {
        method: 'GET',
        headers: {
      ...
      <ComponentCard title="Revenue Trends (Last 7 Days)">
        <div className="p-6">
          <div className="space-y-4">
            {analyticsData.revenue_trends.map((revenue: any, index: number) => {
              const maxRevenue = Math.max(...analyticsData.revenue_trends.map((r: any) => r.revenue));
          ...
          <div className="p-6">
            <div className="space-y-4">
              {Object.entries(analyticsData.revenue_by_service).map(([service, revenue]) => {
                ...
                const totalRevenue = Object.values(analyticsData.revenue_by_service).reduce((sum: number, val: unknown) => sum + (val as number), 0);
          ...
          <div className="p-6">
            <div className="space-y-4">
              {Object.entries(analyticsData.payment_methods).map(([method, count]) => {
                ...
                const totalCount = Object.values(analyticsData.payment_methods).reduce((sum: number, val: unknown) => sum + (val as number), 0);
          ...
          <div className="p-6">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {analyticsData.top_customers.map((customer: any, index: number) => (
