...
export default function BookingAnalyticsPage() {
  ...
  const [analyticsData, setAnalyticsData] = useState<BookingAnalyticsData | null>(null);
  ...
  const loadAnalyticsData = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/v1/admin/analytics/bookings', {
        method: 'GET',
        headers: {
  ...
  return (
    <div className="space-y-6">
        ...
        <ComponentCard title="Booking Trends (Last 7 Days)">
          <div className="p-6">
            <div className="space-y-4">
              {analyticsData.booking_trends.map((trend: BookingTrend, index: number) => {
                const maxCount = Math.max(...analyticsData.booking_trends.map((t: BookingTrend) => t.count));
      ...
      <ComponentCard title="Revenue Trends (Last 7 Days)">
        <div className="p-6">
          <div className="space-y-4">
            {analyticsData.revenue_by_period.map((revenue: RevenueByPeriod, index: number) => {
              const maxRevenue = Math.max(...analyticsData.revenue_by_period.map((r: RevenueByPeriod) => r.revenue));
          ...
          <div className="p-6">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {analyticsData.top_routes.map((route: TopRoute, index: number) => (
