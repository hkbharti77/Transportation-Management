...
const RoutesPage: React.FC = () => {
  ...
  const fetchRoutes = useCallback(async (search?: string, status?: string, routeNumber?: string) => {
    ...
    try {
      ...
      setTotalPages(Math.ceil((routesData.total || 0) / limit));
    } catch (err: unknown) {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      fetchRoutes(searchTerm, statusFilter, routeNumberFilter);
    }, 500); // 500ms debounce

    return () => clearTimeout(timeoutId);
  }, [searchTerm, statusFilter, routeNumberFilter]);
  ...
  const handleStatusToggle = async (routeId: number, currentStatus: boolean) => {
    ...
    try {
      ...
      fetchRoutes(searchTerm, statusFilter, routeNumberFilter); // Refresh the list
    } catch (err) {
        ...
        <div className="overflow-x-auto">
          {loading && routes.length === 0 ? (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="ml-2 text-gray-600 dark:text-gray-400">Loading routes...</span>
            </div>
          ) : (
